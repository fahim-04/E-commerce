...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $cate_id = mt_rand(10000, 99999);
        $added_on = date('M d, Y');
        $slug_url = SlugUrl($cate_name);

        $sql = "INSERT INTO ec_categories (cate_id, cate_name, meta_title, meta_desc, meta_key, slug_url, status, added_on)
                VALUES('$cate_id', '$cate_name', '$meta_title', '$meta_desc', '$meta_key', '$slug_url', '$status', '$added_on')";

        $check = mysqli_query($conn, $sql);

        if ($check) {
            echo "<script type='text/javascript'>alert('Category Added Successfully'); window.location='view-categories.php';</script>";
        } else {
                    ...
                    <div class="col-lg-12">
                        <div class="white_card card_height_100 mb_30">
                            ...
                            <div class="white_card_body">
                                <div class="card-body">
                                    <h4 class="card-title mb-5">Fill category details</h4>
                                    <!-- form -->
